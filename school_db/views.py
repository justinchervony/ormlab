...
def problem_three(request):
    course_instructor = Instructor.objects.filter(id=2).get()
    courses = Course.objects.filter(instructor=course_instructor)
...
def problem_six(request):
    ...
    student_changed = Student.objects.filter(id=student_id).get()

    print(f"Id: {student_changed.pk}")
    print(f"Full Name: {student_changed.first_name} {student_changed.last_name}")
    print(f"GPA: {student_changed.gpa}")
